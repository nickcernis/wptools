...
test("missing files throw error", () => {
  expect(() => {
    p.getHeader("Tested up to", "nopath");
  }).toThrow();
});
...
test("can compare tested up to header against latest WP", () => {
  expect.assertions(1);
  return expect(p.testWordPress(readmePath)).resolves.toMatchObject({
...
test("can compare tested up to header against latest WooCommerce", () => {
  expect.assertions(1);
  return expect(p.testWooCommerce(pluginPath)).resolves.toMatchObject({
    pass: expect.any(Boolean),
    version: expect.any(String),
    tested: expect.any(String)
  });
